...
{
    int choice;
    char then[10] = "";                         //在每一次功能后读入字符串，判断是否继续运行程序
    struct student *head;
    get_record(&head);
    do {
        choice = menu();                        //用于判断要运行的功能
        switch(choice)
        {
...
int menu(void)
{
    ...
    puts("5.exit.");
    scanf("%d", &choice);
    while(choice != 1 && choice != 2 && choice != 3 && choice != 4 && choice != 5) {    //直到输入1~5的数字再返回结果
...
void get_record(struct student **head)
{
